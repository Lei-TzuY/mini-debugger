...
def verify_member(records, member, expected_name, expected_encoding, expected_bits):
    ...
    if name != expected_name:
        raise RuntimeError(
            f"live_bit_fields member mismatch: expected {expected_name}, got {name!r}"
        )
...
def verify_layout(records):
    variable = find_variable(records, "inspect_live_bit_fields", "live_bit_fields")
    type_attr = variable["attrs"].get("type")
    if not type_attr:
        raise RuntimeError("live_bit_fields has no DW_AT_type")
    structure = unwrap_type(
        records,
        type_reference(type_attr, "live_bit_fields"),
        "live_bit_fields",
    )
    if structure["tag"] != "DW_TAG_structure_type":
        raise RuntimeError("live_bit_fields is not a compiler structure type")
    size = structure["attrs"].get("byte_size")
    if not size or scalar_number(size, "live_bit_fields byte size") != 4:
        raise RuntimeError("live_bit_fields is not exactly four bytes")
    ...
    if len(members) != 2:
        raise RuntimeError(
            f"live_bit_fields requires exactly two direct members, found {len(members)}"
        )

    signed = verify_member(records, members[0], "signed_bits", 5, 5)
    unsigned = verify_member(records, members[1], "unsigned_bits", 7, 6)
    signed_absolute = signed[1] if signed[0] == "data_bit_offset" else signed[3]
    unsigned_absolute = unsigned[1] if unsigned[0] == "data_bit_offset" else unsigned[3]
    if signed_absolute != 0 or unsigned_absolute != 5:
        raise RuntimeError(
            "live_bit_fields compiler bit locations changed: "
...
def main():
    if len(sys.argv) != 2:
        raise SystemExit("usage: live_bitfield_dwarf_oracle.py <fixture>")
    path = sys.argv[1]
    ...
    print(
        "live bit-field DWARF oracle passed: "
        "type=LiveBitFields bytes=4 signed_bits=5@0 unsigned_bits=6@5 "
...
if __name__ == "__main__":
    try:
        ...
        print(f"live bit-field DWARF oracle failure: {error}", file=sys.stderr)
        raise SystemExit(1)
